...
- `Moving` 구간에서는 버튼 down/up을 삼켜 원본 클릭이 앱에 전달되지 않게 한다. 이동 메시지는 커서 갱신을 위해 통과시킨다(down을 못 받은 앱은 move만으로 클릭·드래그 오동작을 일으키지 않는다). 릴리스가 다른 창의 비클라이언트 영역에서 일어나면 `NCLBUTTONUP`으로 오므로 종료 조건에 포함한다. 위치 지정은 네이티브 루프처럼 매 이벤트마다 동기로 적용한다. 비동기 지정은 대상 큐에 요청이 쌓여 뒤처지므로 쓰지 않는다. 좌표는 최신 커서(`GetPhysicalCursorPos`)를 써서 밀린 lag을 막고, 속도는 이벤트율에 자동 추종한다(주사율 수동 설정 없음). 적용은 디스플레이 주사율(드래그 시작 모니터에서 OS 조회, 실패 시 무제한)을 넘지 않게 합치고, 마지막 요청이 실제 반영됐는지(`GetWindowRect`) 확인된 때만 다음을 낸다(self-clocking). 비동기 지정은 이때 큐에 쌓이지 않으므로 후크 스레드를 막지 않고, 느린 펌프의 창 뒤에서도 drain 속도에 자동 추종한다. 강제 재동기화 100ms로 확인 놓침에 대비한다. 근본 한계: 네이티브 드래그는 대상 스레드 자체가 모달 루프를 돌며 앱 작업을 멈추지만, 우리는 스레드 간 요청이라 바쁜 소유 스레드만큼만 빨라질 수 있다.
- AnyMove 이동 중 `Shift`가 새로 눌리면(상승 엣지) FancyZones 가상 드래그를 연다(FancyZones 호환). 합성 입력으로는 대상이 모달 루프를 열지 않음이 로그로 확인됐으므로(`posted=True`인데 `MOVESIZESTART` 없음), 루프 대신 FancyZones 발동 체인인 WinEvent를 직접 쏜다. `MOVESIZESTART`를 알리면 존이 표시되고, 우리 `SetWindowPos`가 만드는 실제 `LOCATIONCHANGE`를 타고 하이라이트가 따라오며, 버튼을 떼면 `MOVESIZEEND`에 FancyZones가 직접 스냅한다. 우리 이동은 처음부터 끝까지 계속되므로 멈춤이 없고, FancyZones가 없으면 이벤트가 버려져 일반 이동·드롭과 동일하다. 종료(up·Esc·비활성화·Dispose)는 한 곳(`EndSession`)에서 `MOVESIZEEND`를 보장한다. 누른 채 시작했다면 첫 이동에서 바로 열리고, 가상 드래그 중 재요청은 하지 않는다. `Shift+타이틀바`는 네이티브에 그대로 맡긴다.
