...
- 실제 드래그가 끝난 뒤의 `Win-up`에서만 무해한 `F24` 탭 1회를 `SendInput`으로 넣어 눌렀던 흔적을 지워 시작 메뉴가 뜨지 않게 하고, 실제 `up`은 통과시켜 래치를 푼다. 순서가 뒤바뀌어도 시작 메뉴가 뜰 뿐 고착·팬텀 조합은 생기지 않는다(안전한 방향으로만 실패). `F24`는 바인딩이 사실상 없고 고정 키 대화상자도 유발하지 않아 선택했다.
...
- 로그온 시 탐색기보다 먼저 뜨면 아이콘 등록이 실패하므로, 메시지 전용 창으로 `TaskbarCreated`를 받아 아이콘을 재등록한다(탐색기 재시작에도 대응).
